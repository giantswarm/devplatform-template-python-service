...
def create_app(test_config=None):
    ...
    def get_album(aid: int):
        try:
            ...
            return render_template('error.html', error="Album not found"), HTTPStatus.NOT_FOUND
    ...
    def add_album():
        ...
        new_album = Album(0, new_album_json['title'], new_album_json['artist'], new_album_json['price'])
        store.add(new_album)
        return render_template('albums.html', albums=store.list())
    ...
    def update_album(aid: int):
        ...
        a = Album(album_json['id'], album_json['title'], album_json['artist'], album_json['price'])
        store.update(a)
        return render_template('albums.html', albums=store.list())
